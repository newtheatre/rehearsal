...
Thin mirror: `id` text PK (canonical auth id) · `email` unique · `name` · `updated_at`. Upserted by `ensureLocalUser` in the auth middleware. Email is for pickers and notifications only — never exposed via the API.
...
`site_config`: `key` PK · `value` — `warning_window_days` (60), `academic_year_end` (`09-30`), `session_edit_window_days` (14), `notifications_mode` (`dry-run`|`live`).
