...
| `DELETE /api/sessions/:id/signup` | session | withdraw. Allowed until the session is delivered or cancelled, including while the register is open. Closes that person's practice windows for the session, and nobody else's. Returns `promoted`, how many people it moved into a place **and told**: the promotion email is at-most-once per person per session, so a withdrawal racing another one never sends a second copy ([scheduling-design.md](scheduling-design.md) §8) |
...
| `GET /api/module-requests` | session | your own requests, paged (`limit` default 50, max 100) and returned with `hasMore`, plus `board` if you lead a department. The board is `{ modules, hasMore }`: the 25 busiest modules by open request count, each carrying the full `openCount`, the first 10 requesters by request date, and `requestersNotShown` for the rest. The count is computed in SQL over every open request, so it is the whole demand and not the length of the list under it |
...
| `POST /api/_hooks/auth/last-activity` | `{ userIds }` → latest of: last record awarded, last sign-up, last session attended, last session booked or delivered, per user. **Never a timestamp in the future**: a session's own date counts only once it has arrived, so a register marked early reports the sign-up rather than a day nobody has reached. The caller ages an account from this answer, and a future one defers its retention sweep ([gdpr-retention.md](gdpr-retention.md)) |
