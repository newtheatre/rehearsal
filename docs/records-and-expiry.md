...
A certificate's own expiry must be after the date it was awarded and no more than ten years past it, the same bounds a sign-off override is held to: 120 months is the catalogue's own cap on `expiry_months`, so a certificate cannot express a policy a module is not allowed to have ([ADR-0012](decisions/0012-explicit-expiry-marked-on-the-record.md)). Both routes refuse anything outside those bounds with a 400, because an external record is marked overridden and so is permanently outside the recalculation: a mistyped century would read VALID until somebody noticed by eye.
